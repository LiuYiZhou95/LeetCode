...
{
    std::cout << "1.call sharedptr's constructer: counter = " << *counter << std::endl;
}
...
SharedPointer<T>::SharedPointer(const SharedPointer &ptr)
{
    ...
    obj = ptr.obj;
    *counter += 1;
    std::cout << "2.call sharedptr's copy constructer: counter = " << *counter << std::endl;
}
...
SharedPointer<T>& SharedPointer<T>::operator= (const SharedPointer<T> &ptr)
{
    ...
    *counter -= 1;
    ...
    obj = ptr.obj;
    counter = ptr.counter;
    std::cout << "3.call sharedptr's operator: counter = " << *counter << std::endl;
    return *this;
}
template<class T>
